...
class Cda2Neo4j(BaseMiner):
    ...
    def build_dependencies(self, container):
        ...
        for package in container:
            logger.info(f"Ongoing Package: {package.attrib[NAME]}")
            cls: Et.Element
            for cls in package:
                class_id, class_type = self.class_id[cls.attrib[NAME]]
                dependencies = cls.find("dependencies")

                logger.info(f"\tOngoing Class: {cls.attrib[NAME]}")
                logger.info(f"\tTotal Dependencies: {dependencies.attrib['count']}")
                ...
                for dep in dependencies:
                    ...
                    try:
                        ...
                        out = self.session.run(create_node(node_type="Builtin", name=name))
                        dep_id = out.single()[0]
                        dep_type = "Builtin"
    ...
    def parse_package(self, package: Et.Element, container_id):
        ...
        logger.info(f"Ongoing package: {name}, Total Classes: {classes}")
    ...
    def parse_container(self, container: Et.Element):
        ...
        logger.info(f"Ongoing container: {name}")
