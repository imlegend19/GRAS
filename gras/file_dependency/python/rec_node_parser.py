...
from gras.file_dependency.python.node_types import AttributeTree, CallTree, FunctionTree, Class, Decorator, Function
...
class FileAnalyzer(ast.NodeVisitor):
    ...
    @staticmethod
    def create_analyzer_component(node):
        FileAnalyzer().visit(node)
        FileAnalyzer().gen()
    ...
    def __parse_decorators(lst):
        ...
        for decorator in lst:
            ...
            elif isinstance(decorator, Function):
                dic = {
                    "type" : Decorator(subtype=FunctionTree),
                    "value": FunctionTree(node=decorator),
                    "line" : decorator.lineno
                    }
            else:
